  ...
  applyIconColors() {
    ...
    iconSelectors.forEach(selector => {
      ...
      icons.forEach(icon => {
        ...
        const hasSpecificColor = icon.style.color && 
          !icon.style.color.includes('rgb(26, 77, 46)') &&
          !icon.style.color.includes('#1a4d2e') &&
          icon.style.color !== '';

        const isArrow = /fa-arrow-(up|down|left|right)/.test(icon.className);
        if (!hasSpecificColor && !isArrow) {
    ...
    progressBars.forEach(bar => {
      if (!bar.style.background || bar.style.background.includes('green-400')) {
        bar.style.background = 'linear-gradient(90deg, #1a4d2e, #0d2818)';
        bar.style.setProperty('background', 'linear-gradient(90deg, #1a4d2e, #0d2818)', 'important');
      }
    });
  }
